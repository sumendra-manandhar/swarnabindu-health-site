...
export class OfflineStorage {
  ...
  static saveRegistration(registrationData: any): string {
    ...
    try {
      ...
      localStorage.setItem(this.REGISTRATION_KEY, JSON.stringify(existingRegistrations))
      return id
    } catch (error) {
  ...
  static saveScreening(screeningData: any): string {
    ...
    try {
      ...
      localStorage.setItem(this.SCREENING_KEY, JSON.stringify(existingScreenings))
      return id
    } catch (error) {
  ...
  // Sync status methods
  static setSyncStatus(status: { lastSync?: string; isOnline: boolean; pendingCount: number }): void {
    if (!this.isLocalStorageAvailable()) {
      return
    }
  ...
  static getSyncStatus(): { lastSync?: string; isOnline: boolean; pendingCount: number } {
    if (!this.isLocalStorageAvailable()) {
      return {
  ...
  static getPendingSyncCount(): number {
    return this.getUnsyncedRegistrations().length + this.getUnsyncedScreenings().length
  }
  ...
  static clearSyncedData(): void {
    ...
    try {
      const registrations = this.getOfflineRegistrations().filter((r) => !r.synced)
  ...
  static async autoSync(): Promise<void> {
    ...
    try {
      console.log(`Syncing ${pendingRegistrations.length} registrations and ${pendingScreenings.length} screenings`)
      ...
      if (result.success) {
        ...
        pendingRegistrations.forEach((reg) => this.markRegistrationSynced(reg.id))
        pendingScreenings.forEach((screening) => this.markScreeningSynced(screening.id))
